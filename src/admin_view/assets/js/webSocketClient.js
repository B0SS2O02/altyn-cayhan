...
function connect() {
    var ws = new WebSocket('wss://hazynadoner.com.tm/websocket');
    ws.onmessage = async function (event) {
        ...
        if (window.location.pathname === '/admin/orders') {
            if (item.event == 'order') {
                ...
                let tbody = document.getElementsByTagName('tbody')[0],
                    text = `
        <tr class="border-2 border-[#DBDFEA] border-solid ${item.status === 'completed' ? "completed" : item.status === 'cancelled' ? "cancelled" : "waiting"}">
                ...
                <div>
                    <select name="status"
                        ...
                        ${staticStatusOrder.map(status =>
                            `
                            <option  data-key="${status}"  value="${status}" ${status === item.status && "selected"}
                                >${status}
                            </option>
                            `
                        ).join("")} 
            ...
            <td class="border-t-2 text-[red] cursor-pointer"
                onclick="openModal('/api/v1/order/',${item.id},'Delete Order',2,this)" >
        ...
        `
                ...
                const dataOrderId = document.querySelector(`[data-orderid="${item.data.orderId}"]`)
                if (dataOrderId) {
                    const textPaymentStatus = dataOrderId.querySelector('span')

                    if (textPaymentStatus)
                        textPaymentStatus.textContent = "-" + paymentStatus[item.data.status] + "-"
                }
            }
        } else {
            ...
            const textt = `
                <li>
                    ...
                    <p>${format(item.createdAt.trim())}</p>
                    </div>
                </li>
            `;
            notificationOrders += notificationOrders.insertAdjacentHTML("afterbegin", textt)
        }
        const isViewedIds = [item.id]
        await fetch("https://hazynadoner.com.tm/api/v1/order", {
            method: 'PUT',
