...
const findOrderById = async (id, lang = 'tm') => {
                ...
                include: [
                    {
                        ...
                        include: [{ model: ProdTranslation, attributes: ['title', 'description', 'lang'], where: { lang: lang } }]
                    }
                ]
            }
        ]
    ...
    convSequelizeOrder.orderItems.map((data, index) => {
        Object.entries(...data.product.prodTranslations).forEach((element, index) => {
...
const saveOrder = async (body, seqTrans) => {
    ...
    const deliver = JSON.parse(fs.readFileSync(path.resolve('delivery.json'), 'utf8'))
...
const bankUrlRedirect = {
    ...
    registerUrl: function (bankId, orderId, amount, language) {
        ...
        return `${this.banks[bankId]}/register.do?userName=${this.auth[bankId].userName}&password=${this.auth[bankId].password}&orderNumber=${orderId}&amount=${amount}00&language=${language}&currency=934&returnUrl=https://hazynadoner.com.tm/api/v1/order/payment/callback-handler`
    },
...
const createForms = async (order, seqTrans, language = 'ru') => {
    try {
        ...
        let response, data = {}
        if (order.bank == '3') {//that means client choose tfeb bank(wneshekonomicheskiy bank)
            console.log('wnesh')
            ...
            response = await fetch(bankUrlRedirect.registerUrl(order.bank, newOrderUUID, order.totalPrice, language));
            response = await response.json()
            console.log(response)
            data.url = response.formUrl
            data.finalUrl = `https://hazynadoner.com.tm/api/v1/order/payment/callback-handler?orderId=${response.orderId}`
            data.checkOrder = `https://hazynadoner.com.tm/api/v1/order/payment/check-status?orderId=${response.orderId}`
        }
        ...
        return data
        //  { url: response.formUrl, finalUrl: `https://hazynadoner.com.tm/api/v1/order/payment/callback-handler?orderId=${response.orderId}`, checkOrder: `https://hazynadoner.com.tm/api/v1/order/payment/check-status?orderId=${response.orderId}` }
    } catch (err) {
        console.log(err)
        throw new ErrorException('Something went wrong please try again, Message:' + err.message)
    }
}
...
const checkOrder = async (transactionId, language = 'ru') => {
    ...
    if (order.paymentMethod != 'card') {
        console.log('something went wrong order this id=>', order.id, 'payment method cash but card ');
    }

    let response = await fetch(bankUrlRedirect.checkOrderUrl(order.bank, transactionId, language))
    response = await response.json()
    if ((response.ErrorCode == 0 || response.errorCode == 0) &&
        ...
        if (response.OrderStatus == 2 || response.orderStatus == 2)
            ...
            return { success: false, message: 'Tölegiňiz geçmedi.Tazeden synanyşmagyňyzy haýyş edýäris ' }
        else if (response.orderStatus == 6 || response.OrderStatus == 6)
            return { success: false, message: 'Tölegiňiz edilmeli wagty geçdi.Tazeden synanyşmagyňyzy haýyş edýäris' }
...
module.exports = {
    ...
    createForms,
    changeIsViews,
    deleteOrderById, updateStatusOnly
}
